...
def select_name_and_motto_of_char_with_longest_motto
  return "SELECT name, MAX(motto) FROM characters GROUP BY name ORDER BY MAX(motto) LIMIT 1;"
end
...
def select_character_names_and_number_of_books_they_are_in
  return "SELECT Characters.name, COUNT(books.title) FROM Characters JOIN Character_books ON Characters.id = Character_books.character_id INNER JOIN Books ON Books.id = Character_books.book_id GROUP BY Characters.name ORDER BY COUNT(books.title) DESC;"
end
