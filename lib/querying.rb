...
def select_character_names_and_number_of_books_they_are_in
  return "SELECT Characters.name, COUNT(books.title) FROM Characters JOIN Character_books ON Characters.id = Character_books.character_id INNER JOIN Books ON Books.id = Character_books.books_id;"
end
